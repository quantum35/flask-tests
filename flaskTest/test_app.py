...
class TestFlaskApp(unittest.TestCase):
    ...
    def test_modifyMeal(self):
        response = self.app.post('/modifyMeal', data = json.dumps(self.data1) , content_type = 'application/json')
        result = json.loads(response.data)
        self.assertEqual(result["meal_id"], "meal1")
...
#Cutomers
    ...
    def test_changeMealChoice(self):
        response = self.app.post('/modifyMeal', data = json.dumps(self.data4) , content_type = 'application/json')
        result = json.loads(response.data)
        self.assertEqual(result["name"], "ugali")
        self.assertEqual(result["price"], "1000")
        self.assertEqual(response.status_code, 201)
